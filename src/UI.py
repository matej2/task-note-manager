...
from customtkinter import DISABLED, LEFT, WORD, CTkFrame as Frame, CTk as Tk, CTkButton as Button, CTkTextbox as Text, CTkLabel as Label, CTkScrollbar as Scrollbar, CTkToplevel as Toplevel
...
class UI(Frame):
    ...
    def __configure_button_widgets(self):
        ...
        self.submit_button.configure(text="Submit", font=self.__get_font_config())
        self.submit_button.grid(row=0, column=0, padx=120)

        self.open_file.configure(text="Open file", font=self.__get_font_config())
        ...
        self.export_button.configure(text="Export data - ODS", font=self.__get_font_config())
    ...
    def __configure_input_widgets(self):
        ...
        self.done_field.configure(font=self.__get_font_config(), height=4, width=30, wrap=WORD)
        ...
        self.in_progress_field.configure(font=self.__get_font_config(), height=4, width=30, wrap=WORD)
        ...
        self.problems_field.configure(font=self.__get_font_config(), height=4, width=30, wrap=WORD)
    ...
    def __configure_status_widgets(self):
        ...
        h = Scrollbar(self.task_list_container, orientation='horizontal', border_spacing=2)
        h.grid(row=2, column=0, sticky='nsew')
        v = Scrollbar(self.task_list_container, border_spacing=2)
        v.grid(row=1, column=1, sticky='nsew')

        self.task_list.configure(width=30, height=15, wrap=WORD,
        ...
        h.configure(command=self.task_list.xview)
        v.configure(command=self.task_list.yview)
    ...
    def __configure_scheduler_widgets(self):
        ...
        self.counter.configure(font=self.__get_font_config(), text="Remaining time: --:--")
        self.counter.grid(row=0, column=0)

        self.notification.configure(text_color="red", font=self.__get_font_config())
